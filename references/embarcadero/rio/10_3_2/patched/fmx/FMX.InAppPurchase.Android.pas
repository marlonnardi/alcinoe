  ...
  ConsumeThread := TThread.CreateAnonymousThread(procedure
    ...
    begin
      try
        ...
        TAndroidApplicationGlue.Current.NativeActivity.vm^.DetachCurrentThread(TAndroidApplicationGlue.Current.NativeActivity.vm);
      end;
    end);
  ConsumeThread.Start;
end;
...
function TIabHelper.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID, Obj);
  if (FContext <> nil) and (Result <> S_OK) then
    Result := FContext.QueryInterface(IID, Obj);
  if (FService <> nil) and (Result <> S_OK) then
    Result := FService.QueryInterface(IID, Obj);
  if (FServiceConn <> nil) and (Result <> S_OK) then
    Result := FServiceConn.QueryInterface(IID, Obj);
end;
...
procedure TIabHelper.onBindingDied(name: JComponentName);
begin
end;
...
begin
  if IabResult.IsFailure then
    DoError(TFailureKind.Purchase, Format('%s: %s', [SIAPSetupProblem, IabResult.ToString]))
  else
  begin
...
begin
  Result := inherited QueryInterface(IID, Obj);
  if (FHelper <> nil) and (Result <> S_OK) then
    Result := FHelper.QueryInterface(IID, Obj);
end;
