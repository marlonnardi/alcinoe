...
var
  MediaControlsSavedParents: TDictionary<Pointer, Pointer>; // <TControl, TControl>, we use pointer for ARC ompatibility.
...
begin
  MediaControlsSavedParents.Remove(Self);
...
var
  ...
  begin
    Result := (SavedParent = AForm) or
              (SavedParent <> nil) and (TControl(SavedParent).Root <> nil) and (TControl(SavedParent).Root.GetObject = AForm) and (AForm is TCommonCustomForm);
  end;

begin
  MediaControlsSavedParents.TryGetValue(Self, SavedParent);

  if (FMediaPlayer <> nil) and IsMediaRootForm(Sender) then
  begin
    Parent := TControl(SavedParent);
    UpdateMedia;
  end;
end;
...
begin
  ...
  MediaControlsSavedParents.AddOrSetValue(Self, Parent);
  Parent := nil;
end;
...
initialization
  MediaControlsSavedParents := TDictionary<Pointer, Pointer>.Create;
  RegisterAliases;
  RegisterFmxClasses([TCameraComponent, TMediaPlayer, TMediaPlayerControl]);
finalization
  FreeAndNil(MediaControlsSavedParents);
  UnregisterAliases;
