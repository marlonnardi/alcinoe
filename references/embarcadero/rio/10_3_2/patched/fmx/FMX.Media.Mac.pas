...
begin
  inherited;
  for I := 0 to TQTCaptureDevice.OCClass.inputDevicesWithMediaType(QTMediaTypeSound).count - 1 do
  begin
  ...
  for I := 0 to TQTCaptureDevice.OCClass.inputDevicesWithMediaType(QTMediaTypeVideo).count - 1 do
  begin
...
begin
  ...
  begin
    case Prop of
      ...
      TProperty.Name: Result := NSStrToStr(FDevice.localizedDisplayName);
      TProperty.UniqueID: Result := NSStrToStr(FDevice.uniqueID);
    else
      Result := '';
    end;
  end
...
const
  ...
  BitsPerPixel = 32;
var
  BytesPerRow, Width, Height, BufferSize: size_t;
  BaseAddress: Pointer;
        ...
        try
          ...
          try
            ...
            if ContextRef = nil then
              raise EVideoCaptureFault.Create(SVideoCaptureFault);
            try
              CGContextDrawImage(ContextRef, CGRectMake(0, 0, ABitmap.Width, ABitmap.Height), ImageRef);
            finally
